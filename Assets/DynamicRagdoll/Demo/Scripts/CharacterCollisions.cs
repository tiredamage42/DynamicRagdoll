    ...
    public class CharacterCollisions : MonoBehaviour
    {
		...
		*/
		...
		void UpdateTriggerTrackers () {
            ...
            */
            ...
            inCollisionDetector.enabled = ragdollController.state == RagdollControllerState.Animated || ragdollController.state == RagdollControllerState.BlendToAnimated;
		}
		...
		*/    
		void OnRagdollCollision(RagdollBone bone, Collision collision)
		{
			...
			*/
            ...
            if (checkForRagdoll) {
                ...
                // if the collision is below our incoming threhsold, 
                // check if we're travelling fast enough to go ragdoll

                if (collisionMagnitude2 < incomingMagnitudeThreshold * incomingMagnitudeThreshold){
                    message = "outgoing";
                    
                    collisionMagnitude2 = controllerVelocity.sqrMagnitude;
                    
                    if (collisionMagnitude2 < outgoingMagnitudeThreshold * outgoingMagnitudeThreshold){
                        return;
                    }
                }
        ...
        void HandleBoneDecayOnCollision (float collisionMagnitude2, RagdollBone bone, Collision collision) {
            ...
            if (collisionMagnitude2 >= decayMagnitudeRange.x * decayMagnitudeRange.x) {
